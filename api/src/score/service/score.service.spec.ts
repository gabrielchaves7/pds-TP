...
describe('ScoreService', () => {
  ...
  const spyScoreDataSource = () => 
    jest
      ...
      const spyGetScoreDataSource = () => 
      jest
        .spyOn(scoreDataSource, 'get')
        .mockImplementationOnce(() => Promise.resolve([new Score()]));
  ...
  describe('GET first 5 Scores order by date', () => {
    it('should call scoreDataSource.get', async () => {      
      spyGetScoreDataSource();
      const getSpy = jest.spyOn(scoreDataSource, 'get');
      await scoreService.get();
